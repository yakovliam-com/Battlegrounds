...
public class Message {
    ...
    public static void initAudience(BattlegroundsPlugin plugin) {
    ...
    public void broadcast(String... replacers) {
        msg(new ArrayList<>(Bukkit.getServer().getOnlinePlayers()), replacers);
    }
    ...
    public Component toComponent(String... replacers) {
        ...
        for (String text : lines) {
            ...
            do {
                ...
                if (i1 != -1) {
                    componentBuilder.append(LegacyComponentSerializer.legacyAmpersand().deserialize(text.substring(i2 + 1, i1)));

                    i2 = text.indexOf(DELIMITER, i1 + 1);
                    if (i2 != -1) {
                        Component extras = LegacyComponentSerializer.legacyAmpersand().deserialize(text.substring(i1 + 1, i2));
                        ...
                        if (extra.action != null) {
                            switch (extra.action) {
                                case RUN_COMMAND -> eventComponentBuilder.clickEvent(ClickEvent.clickEvent(ClickEvent.Action.RUN_COMMAND, extra.content));
                                case SUGGEST -> eventComponentBuilder.clickEvent(ClickEvent.clickEvent(ClickEvent.Action.SUGGEST_COMMAND, extra.content));
                                case OPEN_URL -> eventComponentBuilder.clickEvent(ClickEvent.clickEvent(ClickEvent.Action.OPEN_URL, extra.content));
                            }
                        }
